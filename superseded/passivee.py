...
def main():
    ...
    for panel in panel_ors:
        ...
        y_axis_moment_dam = []
        y_axis_speed = []
        bus_location = 1 * sun.x # location in AU referenced on the sun frame.
        ...
        while True: 
            ...
            if c > 1 and threshold(np.mean(np.diff(y_axis_theta[-convergence_mean:])), 0, thresh=0.0001):
...
def draw_scene(panel_ors, sat):
    ...
    for index,panel in enumerate(panel_ors):
        ...
        plt.plot(panel_vectors[index][0],panel_vectors[index][1], color + "-")
        plt.plot(panel_vectors[index][0],panel_vectors[index][1],'ob')
        #plot symmetry
        plt.plot(-1*np.array(panel_vectors[index][0]),panel_vectors[index][1], color + "-")
        plt.plot(-1*np.array(panel_vectors[index][0]),panel_vectors[index][1],"ob")
