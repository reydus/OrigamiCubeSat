...
class Panel:
    def __init__(self, frame, dimensions=[10], connections=[], state=0):
        self.state = state
        self.dimensions = [10] # dimensions[0]
...
def main():
    theta_zero = 0.005 * sp.pi #-2/3 * sp.pi # 30/180 * sp.pi#10/180 * sp.pi
    ...
    for panel in panel_ors:
        ...
        y_axis_moment_dam = []
        y_axis_speed = []
        bus_location = 0.39 * sun.x # location in AU referenced on the sun frame.
        omega_0 = 0 * panel.frame.z
        spring_trigger= 0
        debugBreak = False
        while True: 
            
            f = panel.area * calc_srp(bus_location, bus) * abs(mech.dot(bus.x, panel.frame.x)) #(1+panel.state)
            if get_angle(panel.frame, bus) > (0.5 * sp.pi).evalf(): # if it is shadowed by the hub, there is no force being exerted.
                f = 0
            ...
            '''
            '''
            if get_angle(panel.frame, bus) < (-2/3 * sp.pi).evalf(): # Once panel reaches central rod, exert opposite equal resistance and stop it from moving.
                panel.torque = 0 * panel.frame.z
                alpha = alpha * 0
                d_theta = d_theta * 0
                omega_0 = omega_0 * 0
             '''
            alpha = (panel.torque+spring+damper) / panel.MOI
            alpha = mech.dot(alpha, panel.frame.z).evalf() * panel.frame.z
            ...
            print(str(mech.dot(theta, panel.frame.z).evalf())+", "+str(mech.dot(alpha, panel.frame.z).evalf())+", "+str(c))
            ...
            if get_angle(panel.frame, bus) > sp.pi.evalf()/2:
...
def get_spring_moment(frame1, frame2, alpha=0):
    k = 1.43E-2 # spring constant. N*M/rad
    ref_spring_theta = 30/180 * 3.14159265 # rads
